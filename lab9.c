...
void insertRecord(struct HashType *hashTable, struct RecordType *record, int tableSize)
{
    ...
    struct RecordType* curr = hashTable[index].current;

    if (curr == NULL) 
    {
        hashTable[index].current = record;
    }
...
void displayRecordsInHash(struct HashType *pHashArray, int hashSz)
{
    // for each entry in the table
        ...
        // The output should look like this (for example): "Index 1 -> 21715, Q, 16 -> 28876, a, 26 -> NULL"
         ...
         for (int x = 0; x < hashSz; x++) 
         {
            if (pHashArray[x].current != NULL)
             {
                printf("Index %d ->", x);
                struct RecordType * element = pHashArray[x].current;
                while (element != NULL) 
                {
                    printf(" %d, %c, %d ->", element->id, element->name, element->order);
...
int main(void)
{
    ...
    {
        hashTable[x].current = NULL;
    }
    for(int x = 0; x < recordSz; x++)
    {
        insertRecord(hashTable, &pRecords[x], hashSize);
    }
